			...
			foreach ($ref->getAnnotations() as $ann => $values) {
				if ($ann === 'property' || $ann === 'property-read') {
					foreach ($values as $tmp) {
						...
						if ($matches === NULL) {
							throw new YetORM\Exception\InvalidStateException('Invalid property definition - "@property[-read] <type> $<property> [-> <column>][ <description>]" expected, "@' . $ann . ' ' . $tmp . '" given.');
						}
						...
						if (count($types) === 2) {
							...
							if (strcasecmp($types[1], 'NULL') === 0) {
								if ($nullable) {
									throw new YetORM\Exception\InvalidStateException('Invalid property type definition - only one NULL is allowed, "' . $matches['type'] . '" given.');
								}
							...
							if (!$nullable) {
								throw new YetORM\Exception\InvalidStateException('Invalid property type definition - multiple non-NULL types detected.');
							}
						}
